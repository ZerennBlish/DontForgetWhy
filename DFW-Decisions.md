...
- **Cloud Checkers via Firebase Cloud Function, not WASM or a third-party API** — Three realistic paths for making checkers AI stronger: (a) compile a battle-tested C engine (e.g. Cake, Zenith) to WASM and ship it in the client, (b) use a third-party chess/checkers API (none exist for checkers with a free tier), or (c) deploy the existing TypeScript engine as a Firebase Cloud Function with server-tuned search parameters. Option (c) wins: same language as the client (no C/WASM build toolchain), the engine code is already written and tested, the Firebase plumbing is already paid for (Blaze plan active from Session 30 for Firestore + $300 credit still in the 90-day window), and server CPU cycles are effectively free compared to a user's phone. The Cloud Function runs `getTopMoves(board, turn, 20, 6000, 5)` — more than double the local max depth, double the time budget — and the engine is strong enough at that depth to be competitive with the consumer-grade WASM engines. The trade-off is that offline users lose access to the cloud strength, but the local engine with improved eval (Session 32 mobility/support/formation/endgame scaling additions) is still playable at all five levels. No new native dependency, no build-time complexity, and we can iterate on the engine without pushing an app update.
- **Improved TypeScript engine over C/WASM** — Related to the cloud decision but worth calling out separately. The local checkers engine stays in TypeScript; Session 32 improves its evaluation function (mobility approximation, piece support scoring, connected formation bonus, endgame scaling of king value + center bonus + non-king advancement, king edge + double-corner penalty) rather than swapping the whole thing out for a ported C engine. Rationale: the engine is already maintainable, the test suite (52 existing + new eval/getTopMoves tests) gives us confidence to refactor, and the cloud path is strong enough that the local engine doesn't need to be a world-class checkers engine to serve its role as an offline fallback. A ported C-to-WASM engine would add another dependency, another build step, and another place where an audit finding requires patching a language we don't read as easily.
- **Trivia overhaul: 8 parent categories + 19 subcategories, replacing 10 flat categories** — The Session 22 trivia had 10 flat categories (general, food, kids, geography, history, sports, music, movies, science, entertainment). Two problems: (a) food + kids had no OpenTDB backing — they were bundled-only, silently limiting the "online trivia" promise when those categories were picked; (b) users frequently wanted finer granularity ("I want sports trivia but not board games" / "I want film but not music"). Session 32 restructures into 8 parent categories with 1-4 subcategories each, using the OpenTDB category taxonomy as the design constraint: every parent has an OpenTDB mapping, every subcategory either is an OpenTDB category (for specificity) or aggregates multiple OpenTDB categories (for the "all" case). Parents: general, popCulture (4 subs), scienceTech (4), historyPolitics (3), geography, sportsLeisure (3), gamingGeek (2), mythFiction. The subcategory picker modal opens for multi-sub parents and gives users a second-level pick with live question counts + easy/medium/hard breakdown.
- **Food + kids questions remapped to `general`, not dropped** — The old food and kids categories had 196 combined curated questions. Option (a): drop them as part of the restructure. Option (b): keep them in a "misc" or "other" parent that doesn't exist in the new taxonomy. Option (c): remap them into `general` where they continue to serve as general-knowledge questions. Option (c) wins — the curated work shouldn't be lost, and "general knowledge" already spans a wide topic range, so food/kids-appropriate content fits naturally. The migration is one-time and silent: the bank file for `general` inherits those questions with updated `category` and `subcategory` fields. No user-facing deletion, no lost content.
- **Subcategory picker modal only for multi-sub parents** — Three of the 8 parents have a single subcategory (`general` → `generalKnowledge`, `geography` → `geography`, `mythFiction` → `mythologyBooks`). Opening a picker with exactly one tile ("All {parent}" + one sub that equals the parent) would be redundant tapping. `handleCategoryTap` skips the modal for those parents and treats the tap as an immediate "All {parent}" selection. The `PARENT_TO_SUBS` map is the single source of truth for which parents open the picker — `subs.length > 1` is the only condition, no special-case list.
- **Seen-question composite keys (`category` or `category_subcategory`)** — The storage layer accepts opaque string keys; the TriviaScreen composes them based on the current selection. For "All {parent}" rounds, the key is `{category}`. For specific-sub rounds, the key is `{category}_{subcategory}`. Per-view seen cycles (each "view" has its own recycling) trade off cross-view dedup (a question seen via "All Pop Culture" isn't flagged as seen when the user next plays Film). Considered keyed flat sets (one global seen set keyed by question id only), but rejected — the per-view cycle has UX value: users who play "All Pop Culture" then switch to Film get a mostly-fresh Film experience instead of discovering half of Film is already "seen." Audit flagged the cross-view dedup limitation as P2; logged as intentional for now.
- **Online questions get a default subcategory (first sub of their parent)** — OpenTDB's API supports querying by category but not by subcategory. When a user selects "Film" specifically but online mode is on, the client can't ask OpenTDB for "popCulture → film" — it can only ask for category 11 (which OpenTDB calls "Film" but the DFW schema mentions could plausibly cover any pop-culture subcategory depending on OpenTDB's own taxonomy). Resolution: online requests go by parent category, and the returned questions are tagged with `subcategory = PARENT_TO_SUBS[parent][0]` (the first sub in the parent's list). This makes the data structurally valid without pretending OpenTDB understands DFW's subcategory granularity. Users who pick Film specifically and are online get OpenTDB popCulture questions that may or may not be film — accepted trade-off.
- **All prompts as `.md` files with `@` references (reinforced from Session 31)** — Session 31 established the practice; Session 32 reinforced it by making every prompt (audit, fix, doc update) an `.md` file under `C:\DFW-Context\` with explicit `@file.ext` references at the top for context-loading. Benefit: prompt versions are editable, diffable in git, and reviewable before paste. The alternative — inline-paste into the chat — is faster for one-offs but has no audit trail and no reuse path. For a project where the audit → fix → doc-update cycle happens multiple times per session, the `.md` + `@` pattern is the right investment. Key rule from Session 31: `@path/to/file` on a line by itself means "load this file's current contents as context"; the harness auto-expands the reference before sending. Saves tokens, keeps prompts short, and makes every context reference explicit.
- **No release until v2.0.0 — testing window before paywall activation** — Session 31 prepared v1.23.0 (versionCode 41) with full Pro Tier plumbing; Session 32 added Cloud Checkers + trivia overhaul + globe indicators + IAP retry. Two reasonable options: (a) ship v1.23.0 now with Session 31, then ship v1.24.0 with Session 32, or (b) bundle both into v2.0.0 and ship once. Went with (b): versioning signals "major feature release" to users, the Play Store listing refresh (screenshots, description, what's-new) only needs to happen once, and the combined scope is big enough to warrant the 2.0 marker. Downside: users on v1.22.0 are running production code that's ~2 sessions behind `dev`. Acceptable because no security fixes are involved, and the bundled ship gives the testing window a full feature set to evaluate before paywall activation.
- **Dice game (Yahtzee-style, different name) deferred to post-2.0** — Considered adding a dice game to the games grid as part of Session 32's trivia work. Rejected because (a) it's a net-new feature that would compete with the trivia overhaul for testing attention on the same ship, (b) the name requires research to avoid trademark issues (Yahtzee, Yacht, Generala are all regionally trademarked or commercially licensed in some jurisdictions), (c) the game design itself (scoring categories, reroll limits, bonus thresholds) deserves a dedicated session rather than being a sideline. Backlogged as "Dice game (Yahtzee-style, different name) — post-2.0." A future session will pick this up alongside any multiplayer mechanics — dice is a natural fit for head-to-head play.
- **Multiplayer trivia / chess / checkers deferred to post-2.0** — The Firebase foundation from Session 30 (Auth + Firestore) makes real-time multiplayer tractable, and Session 32's Cloud Checkers function is a proof of concept that we can run game logic server-side. But multiplayer adds matchmaking, presence, turn coordination, timeouts, disconnection handling, and a whole social surface (invites, friends, profiles) that don't exist yet. Rather than bolt multiplayer onto the v2.0 ship, the plan is: ship v2.0 with single-player Cloud Checkers + cloud chess + online trivia, validate the cloud infrastructure in production, then tackle multiplayer as a dedicated post-2.0 feature. Each of the three (trivia / chess / checkers) will likely be its own session because the social primitives they need are different (trivia = turn-based answer-race, chess/checkers = turn-based persistent board).
- **Cloud endpoint auth (Firebase App Check) deferred to pre-Pro launch** — The Session 32 Cloud Checkers function is currently a public unauth HTTPS endpoint — any request with a valid board + turn gets a ranked move back. Fine for v2.0 because (a) the function is compute-bounded with `maxInstances: 10` + `timeoutSeconds: 30` (maximum cost exposure per unit time is small), (b) there's no user data involved (stateless board → stateless moves), and (c) the Firebase $300 credit window extends to July 14, 2026 so quota pressure isn't a near-term concern. But once Pro launches as a paid tier, App Check becomes mandatory to (1) prevent someone from scraping the endpoint for a free Cloud-Stockfish-class engine in their own app, and (2) ensure only real DFW builds can drive the cost. App Check integrates with Play Integrity on Android — the client attaches an attestation token to every function call, and the function rejects requests without valid tokens. The work is scoped (~1 session) and queued for the pre-Pro launch window.
- **OAuth consent screen verification deferred to pre-Pro launch** — Similar logic to App Check. Google requires OAuth consent screen verification for any app that requests sensitive scopes (Calendar read + write both qualify) once the app exceeds 100 users or wants to leave "testing" mode. DFW is currently on a limited testing-mode OAuth config that works for the license testers + closed testing track; production-grade verification requires a privacy policy URL (already have), a homepage URL (already have), a verified domain (have), and a Google review process (~3-5 business days typical turnaround). Not blocking for v2.0 because the testing-mode config still works for the current user count, but scheduled to complete before paywall activation because a user in "testing mode" can see scary "this app is unverified" warnings that hurt conversion.
...
- **Empty voice memos open in edit mode, not view mode** — `VoiceMemoDetailScreen` defaulted to view mode for every memo. View mode renders the title only if non-empty, so a fresh recording (seeded with `title: ''` and `note: ''` by `VoiceRecordScreen.saveAndNavigate`) showed a blank-looking screen with no visual cue that a title field existed. The back-nav discard guard then asked "you didn't add a title" about a UI element the user was never shown. Decision: on mount, if the loaded memo has empty title AND empty note, `setIsViewMode(false)`. Covers fresh recordings AND legacy orphan memos in one rule. Alternative considered: route param (`isNewMemo: true`) passed from `VoiceRecordScreen`, which would have been cleaner intent-signaling but required plumbing through `RootStackParamList` and two more files. Rejected for scope — the "empty means edit" rule catches both cases the route param would distinguish.
- **Voice memo titles remain optional; discard guard is 3-button, not 2-button** — Initial discard guard design was Cancel + Discard. This implicitly forced users to either title the memo or delete it, treating title as mandatory. But DFW has never required titles on voice memos — the audio itself is the primary content. Decision: Alert has three buttons — **Cancel** (stay), **Toss** (delete + leave), **Keep** (leave, memo persists untitled). Matches the existing 3-button pattern of the same file's "Unsaved changes" Alert. Titles remain an optional metadata field, as designed.
- **Empty-memo prevention at state transition, not at exit** — Considered two enforcement points for "no empty memos in DB": (a) extend the back-nav `isDisposable` guard to also check `clips.length === 0` and auto-delete on exit, or (b) intercept the state transition itself — when `handleDeleteClip` removes the last clip of an otherwise-empty memo, cascade the delete to the whole memo. Chose (b). The state transition is the causally-correct place to enforce the invariant; (a) would delete memos users meant to keep if they'd cleared title/note/photos for some flow we didn't anticipate. `handleDeleteClip` computes `shouldDeleteMemo = isLastClip && memoOtherwiseEmpty`, re-titles the Alert ("Goodbye, whole memo?"), and routes confirm through `permanentlyDeleteVoiceMemo` + navigate back. Non-empty-memo clip deletes behave exactly as before.
- **Alert copy belongs to the app's voice, not system chrome** — The initial voice memo Alerts used plain engineering tone ("Discard recording?" / "No title or note yet. Toss the recording, or keep it as-is?"). This clashed with DFW's established personality in the same file (the trash Alert: *"Once it's gone, your brilliant thought goes with it. Well, for 30 days. Then it's really gone."*; the SAVE_TOASTS array; the recording hints). The default engineer instinct to treat Alerts as neutral system chrome is wrong for branded apps — Alerts are part of the voice. Decision: all three of this file's Alerts (Unsaved changes, Unlabeled mystery, Goodbye whole memo) updated to DFW voice. Button labels renamed to match (Stick around / Ditch it / Save it for the Unsaved changes Alert; Cancel / Toss / Keep for the Unlabeled mystery Alert). The "Delete this clip?" non-cascade branch kept plain because destructive buttons benefit from clarity over cuteness. Principle recorded for future sessions: user-facing strings default to the app's voice, not to what's technically correct and minimal.
- **Root `tsc` transitively depends on `functions/node_modules`** — Session 34 mid-session fix for a Metro crash (orphaned npm temp file) deleted `functions/node_modules` to eliminate the bad file. Root `npx tsc --noEmit` then failed with "Cannot find module 'firebase-functions/v2/https'" because the root tsconfig's include pattern picks up `functions/src/*.ts`, and TypeScript's module resolution walks up from the importing file's directory looking for `node_modules` — which means it finds `functions/node_modules` for imports in `functions/src/*`. So `functions/node_modules` isn't just a Cloud Functions thing; it's load-bearing for main app typecheck as long as root tsc sees the `functions/` tree. Fix for tonight: reinstall it. Architectural cleanup queued: add `"exclude": ["functions", "dist"]` to root `tsconfig.json` so root tsc treats `functions/` as an independent sub-project. Not done this session — out of scope, and deserves a dedicated moment to verify nothing else in the project assumes the current behavior.
- **Audit round landed three P2 corrections on voice memo work before push** — The voice memo feature commit + tone pass was pushed to `origin/dev` as `25e3d8c` before triple audit (a rule violation flagged mid-session by the user; the rule is "audit before every ship and every dev→main merge"). Retroactive audit (Codex + Claude + Gemini, all report-only, scoped to Session 34 changes in `VoiceMemoDetailScreen.tsx`) found 3 P2 with consensus and no P1. Corrections applied in a follow-up commit: (1) `isDisposable` extended to treat photos as content (prevents silent photo deletion on Toss of a photographed fresh recording); (2) `memoOtherwiseEmpty` in the cascade switched from live TextInput state to saved initial-refs state (prevents destructive cascade on temporarily-cleared titled memos); (3) `exitingRef.current = true` latched before each `await permanentlyDeleteVoiceMemo` to close the async re-entry race, with delete failures now surfacing via `ToastAndroid` and resetting `exitingRef` so the user can retry (matches the `handleSaveExisting` precedent already in this file). Lesson: pre-push audit is cheaper than post-push correction — the corrective commit added complexity that a fresh audit against a fresh feature would have folded into the initial design. "Audit before ship" is there for a reason, treat push as the ship event even on dev

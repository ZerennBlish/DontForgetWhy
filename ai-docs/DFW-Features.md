...
- **Voice Memos** — standalone VoiceMemoListScreen. VoiceRecordScreen (tap-to-record/stop, pause/resume, camera button). VoiceMemoDetailScreen dual-mode (view/edit, circle Edit/Save icon buttons). **Fresh recordings and any memo with empty title + empty note open in edit mode** so the title/note inputs are visible on arrival. **Back-nav on a memo with no title, no note, and no photos prompts "Unlabeled mystery?"** with Cancel / Toss / Keep — Keep preserves the untitled memo (titles are optional), Toss calls `permanentlyDeleteVoiceMemo`. Photos count as content and keep a memo off the Toss path. **Deleting the last clip of a memo whose saved title + saved note + photos are all empty cascades to full memo deletion** ("Goodbye, whole memo?" prompt), preventing orphan empty rows. The cascade reads the saved title/note (via `initialTitleRef` / `initialNoteRef`), not live TextInput state, so a user who temporarily cleared a titled memo's input can't accidentally trip the cascade. VoiceMemoCard (View-based play/pause, inline progress, capsule pin/delete). Pinning max 4, soft delete with undo. Purple left-border accent (#A29BFE). Calendar shows voice memos as purple dots.
...
- **Trivia** — 1,623 offline questions across **8 parent categories** and **19 subcategories**: General Knowledge; Pop Culture (Film, Music, Television, Celebrities); Science & Tech (Science & Nature, Computers, Gadgets, Mathematics); History & Politics (History, Politics, Art); Geography; Sports & Leisure (Sports, Board Games, Vehicles); Gaming & Geek (Video Games, Comics/Anime); Myth & Fiction (Mythology & Books). Category grid; multi-sub parents open `SubcategoryPickerModal` with live counts + difficulty breakdown. Single-sub parents skip the modal. Difficulty filter + speed selector. 11 custom DFW-style icons in `assets/trivia/`. **Online-first** — `onlineMode` defaults true, pulls fresh round from OpenTDB at `startRound` time when online; all 8 parents have OpenTDB backing. Online questions get default subcategory (first sub of their parent) since OpenTDB doesn't expose subcategory-level queries. Online rounds don't touch `seenQuestionIds`. Failure silently falls through to offline bank.
  - **Multiplayer (Session 39, Pro-gated):** 2-4 player quiz bowl via `multiplayerTrivia.ts` service + `useMultiplayerTrivia` hook + `MultiplayerTriviaGame` component. TriviaScreen mode selector picks `vs CPU` (solo) or `vs Players` (multiplayer). Create Game picks category + subcategory + question count (10/15/20), returns a 6-char code. Join Game takes a code and adds you to the lobby. **Lobby phase:** host sees Start Game button (enabled at 2+ players), guests see "Waiting for host to start…". Up to 4 players. **Question phase:** active player sees their question + 15-second timer; other players see the same question with "{name}'s Turn". Active player submits an answer. **Result phase (3s):** correct → score +1, advance to next question; wrong → pass to next un-attempted player (steal!); all-tried → reveal correct answer, advance. Host's client calls `advanceToNextQuestion` via a 3-second timeout after each result. **Final phase:** scoreboard with winner (or tie). **Host leaves mid-game →** `leaveTriviaGame` promotes `remaining[0]` to host so advancement keeps working; no deadlock. **Leaver was active player →** rotation skips ahead to the next un-attempted player. Exit guard on hardware back with DFW trivia-themed copy. Lobby exit auto-calls `leaveTriviaGame` (host leaves lobby = cancel game for everyone; guest leaves = remove from roster). Turn banner + "STEAL!" label have `accessibilityLiveRegion="polite"`. Haptic feedback gated on `lastAnswer.uid === myUid` so opponents only get sound.
...
- **Chess** — vs CPU, 5 difficulty levels **Easy / Intermediate / Hard / Expert / Master**. **Move priority: opening book → Lichess cloud eval → local minimax.** 104-entry curated opening book (Italian, Ruy Lopez, Queen's Gambit, London, English / Sicilian, French, Caro-Kann, KID, Slav) returns instantly for first 6-10 plies with random pick across 1-3 equivalent moves. Non-book positions query Lichess cloud eval API (`multiPv=5`); client picks from difficulty-specific rank band into top-5 PVs — Easy 2-4, Intermediate 1-3, Hard 0-2, Expert 0-1, Master 0. Every cloud move is Stockfish-strong; difficulty is *move quality*, not search depth. **Offline fallback** runs the local engine (iterative-deepening minimax, quiescence, 100K-entry FEN-keyed TT with mate-score ply adjustment, killer-move ordering, null-move pruning, tapered eval, passed-pawn bonuses, rook on open/semi-open file, min-depth + max-time difficulty with 3× safety ceiling). Free tier gets cloud for every move; **Pro unlocks unlimited rounds** (free gets 3 trial rounds). Defensive `Math.min(Math.max(saved.difficulty, 0), 4)` clamp on restore. In-game roasts via depth-2 sanity check (58-line pool, 5 severity tiers). One take-back per game. State persists to SQLite. Memory Score: 5/8/12/18/25 win points, half for draw.
  - **Multiplayer (Session 39, Pro-gated):** ChessScreen mode selector picks `vs CPU` or `vs Player`. vs Player opens the multiplayer menu (Create Game / Join with Code / resume active game). Create picks color (white/black), hits the `dfw` Firestore `games` collection via `multiplayer.ts` service, returns a 6-char code. Join takes a code and enters the game. Real-time sync via `onSnapshot` — both boards update the instant a move lands. In-game action bar: Offer Draw, Take Break (pause request), Resign. DFW-personality exit guard on hardware back (Keep Playing / Ask for Break / I Quit). Back-from-waiting auto-cancels the game so it doesn't linger in the active list. Game code display has spaced-out TTS label. Per-square `accessibilityLabel` with piece name + state (selected / legal / in check / last move). Post-game move review (FEN history nav) available. Max 5 concurrent active games per player. Both players must be Pro.
- **Checkers** — vs CPU, 5 difficulty levels (Beginner–Expert), American rules (forced captures, promotion ends turn). **Move priority: Firebase Cloud Function → local minimax.** Cloud Function (`checkersai-kte3lby5vq-uc.a.run.app`) runs the TypeScript engine server-side with deeper search (maxDepth 20, timeLimit 6s) and returns top-5 ranked moves; client picks from `cloudPickRange` (Beginner 2-4, Casual 1-3, Intermediate 0-2, Advanced 0-1, Expert 0-0). **Offline fallback**: local minimax (alpha-beta + TT + killers + iterative deepening) with evaluation covering mobility, piece support, connected formation, endgame scaling (scales king value + center bonus + non-king advancement when total pieces ≤ 8), king edge + double-corner penalty. Piece PNGs (red, red-king, black, black-king) + weathered wood table background. No blunder roasts, no take-back. State persists to SQLite. Same Memory Score scale.
  - **Multiplayer (Session 39, Pro-gated):** Identical flow to multiplayer chess — CheckersScreen mode selector, Create Game / Join with Code / resume active game, 6-char codes, real-time `onSnapshot`. Shares the `multiplayer.ts` service. Color adapter maps the shared `hostColor: 'w'|'b'` field to checkers' `'r'|'b'` PieceColor ('w' = red, moves first). Board state serialized as 33-char string (32 dark squares + turn char). Opponent-move sounds derived from piece-count diff (capture on count drop, regular move otherwise). Action bar: Offer Draw, Take Break, Resign. Same DFW-personality exit guard. Per-square `accessibilityLabel` with row/col + piece color/king state. No post-game review mode (asymmetric vs chess — logged as P3). Both players must be Pro.
...
Route param: `{ memoId }`. `VoiceRecordScreen.saveAndNavigate` creates the memo + clip atomically and then `navigation.replace('VoiceMemoDetail', { memoId })` — no `tempUri` flow; the memo already exists by the time this screen mounts. On mount, if the loaded memo has empty title AND empty note, the screen flips `isViewMode` to `false` so the title + note TextInputs are visible immediately ("Add a title…" / "Add a note…" placeholders). This covers both fresh recordings (title/note seeded empty) and legacy orphan memos.

**beforeRemove guard** (three branches in priority order):
1. **Attachments panel open** — close the panel, stay on screen.
2. **`hasChanges`** (title or note differs from `initialTitleRef` / `initialNoteRef`) — "Unsaved changes" Alert, body *"You edited something and didn't save. Save it, ditch it, or stick around?"*, buttons **Stick around** / **Ditch it** / **Save it**. Save path calls `handleSaveExisting` (returns `Promise<boolean>` — false blocks nav).
3. **`isDisposable`** (memo exists AND title + note both empty AND both initial refs empty AND `memo.images` is empty or missing — user never touched the memo and no photos were attached) — "Unlabeled mystery?" Alert, body *"You recorded a thing and gave it zero context. Keep the mystery or toss it?"*, buttons **Cancel** / **Toss** / **Keep**. Toss latches `exitingRef.current = true` *before* awaiting `permanentlyDeleteVoiceMemo(memoId)` (prevents a second back-press from re-entering the guard during the delete); on success, `refreshWidgets()` + `navigation.dispatch`. On delete failure, `exitingRef` is reset to `false` and a `ToastAndroid` "Delete failed — try again" surfaces so the user can retry. Keep exits without deletion. Cancel stays.

**`handleDeleteClip` cascade**: when deleting a clip, if it's the last remaining clip AND the memo's **saved** title is empty AND the **saved** note is empty AND `memo.images` is empty or missing, the Alert re-titles to "Goodbye, whole memo?" with body *"Last clip, no title, no note. Nothing left to hold onto."*. The saved-state check (`initialTitleRef.current` / `initialNoteRef.current`) is deliberate — a user who temporarily cleared a titled memo's TextInput without saving will see the regular "Delete this clip?" path, not the destructive cascade. On cascade confirm, `exitingRef` is latched before awaiting `permanentlyDeleteVoiceMemo(memoId)`; on success, `refreshWidgets()` + `navigation.goBack()`. On failure, `exitingRef` resets and `ToastAndroid` surfaces "Delete failed — try again". Prevents empty memo shells from persisting as side effects of clip cleanup.

Seekable playback (44px touch target wrapping 6px bar, play/pause #4CAF50 64x64, back/forward 5s). Soft delete via trash icon still uses the 30-day pattern (`deleteVoiceMemo`). Note text unlimited; title `maxLength={100}`. All three Alerts (Unsaved changes, Unlabeled mystery, Goodbye whole memo) use DFW voice to match the file's existing trash Alert and SAVE_TOASTS tone — user-facing strings belong to the app's voice, not system chrome.

...
### Nav Button Theming (Session 33)

`BackButton` and `HomeButton` (productivity screens) plus `GameNavButtons` (game screens) consume both `useTheme()` and `useIconTheme()` to drive a chrome-vs-toon visual branch. The discriminator is `isChrome = iconTheme !== 'anthropomorphic'` — chrome and mixed both resolve `backArrow` / `home` to chrome glyphs, so they share the chrome-circle treatment. Anthropomorphic resolves to character art and renders bare.

- **`forceDark` is now functional in `BackButton` and `HomeButton`** — the prop was declared in the Session 32 rewire but unused. Session 33 wired it into the chrome-circle background selector: `isDark = forceDark || colors.mode === 'dark'`. Game screens and the 8 user-photo screens that already pass `forceDark={!!bgUri}` (see §3 "Overlay Text Strategy") now get the dark-bg circle automatically when chrome glyphs render.
- **`GameNavButtons` does not accept `forceDark`** — game screens always sit on a permanent dark photo overlay, so the chrome circle hardcodes `rgba(30,30,40,0.8)`. No mode branching needed at the component level.
- **`useTheme()` import on the productivity buttons** — `BackButton` and `HomeButton` now import `colors` from the theme context to read `colors.mode` for the light-vs-dark bg selector when `forceDark` is unset.

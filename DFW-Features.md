...
- **Trivia** (912+ offline questions across 10 categories incl. Kids/Music/Movies&TV, difficulty filter, speed selector). **Online-first as of Session 31** — `onlineMode` defaults to true and pulls a fresh round from OpenTDB at `startRound` time whenever connectivity is available AND the category has an OpenTDB mapping (food + kids stay offline-only — OpenTDB has no matching category for either). Online rounds don't touch `seenQuestionIds`, so they don't contribute to the offline "seen all, reshuffling" cycle. Online failure (network error, OpenTDB rate-limit, `response_code != 0`, parse error) silently falls through to the offline bank — the worst-case is a standard offline round, never an error screen.

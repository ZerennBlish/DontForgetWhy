...
### Source of Truth · Updated: Session 42 (April 24, 2026) — v2.0.1 ship
...
| | |
|---|---|
| **Current Version** | **v2.0.1 (versionCode 45)** — branding refresh. New grumpy alarm clock mascot as launcher icon and in-app watermark, watermark opacity centralized as theme token, `.gitattributes` + EOL normalization. |
| **Branch** | `dev` — `main` still at v1.24.0, merge pending post-ship. `v2.0.1` tag on dev. |
| **Production Status** | v2.0.1 (versionCode 45) uploading to Play Store. v2.0.0 (versionCode 44) is current installed baseline. |
| **Current Focus** | v2.0.1 shipped. Post-launch monitoring. |
| **Blocked By** | Nothing |
| **Next Action** | Merge dev→main, store screenshot refresh, post-launch monitoring. |
...
- **Merge dev→main** — main is at v1.24.0, needs to catch up to v2.0.1 post-ship.
...
- **Store screenshot refresh** — current 8 screenshots predate v2.0.0 features. Deferred until multiplayer verified in prod.

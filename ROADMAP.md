...
### Source of Truth · Updated: Session 37 (April 19, 2026)
...
| | |
|---|---|
| **Current Version** | **v1.24.0 (versionCode 43)** — LIVE on Play Store. `dev` has significant v1.25.0 cleanup progress (10 commits post-ship). |
| **Branch** | `dev` — `main` reset to match `dev` this session. `v1.24.0` tag moved to correct commit `0ee7100`. |
| **Production Status** | v1.24.0 (versionCode 43) live on Play Store. |
| **Current Focus** | Dead-code audit — Pass 1 (cross-file) complete, Pass 2 chunks 1-2 (screens + hooks) complete, chunks 3-6 queued. Parallel: asset creation for chrome game art (user-driven, offline). |
| **Blocked By** | Nothing |
| **Next Action** | Pass 2 chunks 3-6 when ready, OR asset wire-up when chrome game assets are done. |
...
- **Pass 2 chunk 3 (services + utils, ~40 files)** — Biggest remaining audit chunk. Orphan exports, unused helpers, dead code in the service/utility layer.
- **Pass 2 chunk 4 (components, ~30 files)** — Mid-size audit chunk targeting the component layer for unused props, dead branches, orphan exports.
- **Pass 2 chunk 5 (data + theme + navigation + widget)** — Asset registries, highest risk, worth a fresh session. Check for orphan keys in `APP_ICONS` / `MEDIA_ICONS` / asset maps, dead theme tokens, unused nav types, dead widget props.
- **Pass 2 chunk 6 (functions + plugins, 2-3 files)** — Smallest remaining chunk. Quick sweep of Firebase Cloud Function + config plugin code.

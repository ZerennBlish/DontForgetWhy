...
### Source of Truth · Updated: Session 48 (May 16, 2026) — v2.1.0 ship sync
...
| | |
|---|---|
| **Current Version** | **v2.1.0 (versionCode 47)** — dice roll audio wired across single and multiplayer, five tracked audit fixes resolved (joinGame/joinTriviaGame transactions, cloudCheckers validation tightened, HomeScreen scrollable on small devices, new multiplayerTrivia test file), `runTransaction` mock added to `multiplayer.test.ts`. |
| **Branch** | `dev` and `main` in sync at v2.1.0. |
| **Production Status** | v2.1.0 (versionCode 47) shipped to Play Store on May 12, 2026. dev + main in sync. |
| **Current Focus** | Post-v2.1.0 — backlog selection. |
| **Blocked By** | Nothing. |
| **Next Action** | Schedule voice clip recording session for Alarm Guy character lines (10 new lines drafted Session 47). Pick next backlog item. |
| **EAS Credits** | Reset May 12, full budget available. |
...
| **ElevenLabs** | Subscription active — 84 clips shipped (68 original + 15 tutorial + Opening.mp3). New batch staged: 10 Alarm Guy lines drafted Session 47, awaiting recording. |
...
The active backlog now that v2.0.1 is out. Items pulled forward from the pre-v2.0.0 backlog.
...
- **Store screenshot refresh** — current 8 screenshots predate v2.0.0 features. Multiplayer permission-denied resolved (App Check debug token), so this is unblocked.
...
- **Icon sizing tuning (post-ship polish)** — Session 33 standardized GamesScreen card icons at 48×48 + nav buttons at 36/40 per theme, but `VoiceMemoDetailScreen` media controls, `DrawingCanvas` toolbar, and the `ChessScreen` / `CheckersScreen` header icons were not part of that pass and may want similar bumps when chrome theme is active. Not blocking for v2.0 dormant ship.
- **Orphaned `icon-magnify.webp`** — `assets/icons/icon-magnify.webp` is unreferenced after `DailyRiddleScreen` migrated to `useAppIcon('search')` in Session 32. Cleanup candidate; no behavior impact while it sits on disk.
...
- **TimerScreen + VoiceMemoDetailScreen nav layout inconsistencies** — these screens use bespoke header layouts that don't follow the `BackButton left: 20` / `HomeButton left: 64` productivity pattern Session 33 standardized. Spacing/positioning of the header buttons drifts vs. the rest of the app. Cleanup candidate, not a blocker.
...
| Apr 24 | Session 42: **v2.0.1 branding refresh shipped.** New grumpy alarm clock mascot as Android launcher icon + in-app watermark (binaries replaced in `assets/adaptive-icon.png` and `assets/fullscreenicon.webp`). `watermarkOpacity` theme token added to `ThemeColors` interface — dark themes 0.20 (up from 0.15), light themes 0.15 (up from 0.06) — replacing 13 inline `colors.mode === 'dark' ? 0.15 : 0.06` ternaries across the screen layer (one place to tune going forward). `.gitattributes` added with `* text=auto eol=lf` + explicit binary markers; `git add --renormalize .` swept the working tree to LF in one commit, ending the CRLF/LF drift that was dominating PR diffs. Cloud Functions Node.js runtime upgraded 20 → 22 (deployed and verified). Multiplayer dev-build `permission-denied` diagnosed as App Check debug-token rotation — workflow step (re-register the token after each dev install), no code change needed. Agent docs (CLAUDE.md, AGENTS.md) updated for Node 22, the audit-only / no-`/tmp`-writes rule (Session 37 lesson now standing), and the corrected `src/components/*Modal.tsx` layout (stale `src/modals/` directory reference dropped). 28 suites / 614 tests passing, `npx tsc --noEmit` clean. **v2.0.1 (versionCode 45) uploading to Play Store.** |
| Apr 30 | Session 43: **Doc housekeeping + project kit creation.** Scoped Firebase modular API migration (6 files, ~30 call sites — not executed). `.claude/settings.local.json` gitignored. Project instructions pruned (~90 rules → ~62). User preferences updated. Bald Guy Project Kit created (About-Me, Opus template, CLAUDE template, userPreferences reference). Tagline locked: "Alarms. Notes. Games. Judgment." No code changes, no version bump. |
| May 7 | v2.0.2 (between sessions): **Welcome overlay + Opening.mp3 first-launch experience.** 4-slide guided walkthrough on HomeScreen, auto-advancing every 6.5s timed to the ~26s Opening.mp3 clip. Gated by `kvGet('opening_clip_played')` one-time flag — fixes the long-standing bug where Opening.mp3 triggered on every launch instead of once. `dismissWelcome()` handles cleanup (pause audio, remove listener, clear interval, write flag). Navigation blur listener dismisses the overlay if the user navigates away mid-presentation. Games count corrected to 7 on GamesScreen. Doc restructure: `ai-docs/` folder committed, stale `docs/` predecessor files removed (ARCHITECTURE.md, AUDIT_HISTORY.md, CHANGELOG.md, DATA_MODELS.md), copy-for-claude script updated. Laptop references removed from operational docs (laptop retired from coding). **v2.0.2 (versionCode 46) AAB built.** |
| May 11 | v2.0.2 (versionCode 46) shipped to Play Store, including the That One Dice Game work from Session 46 (Yahtzee-style, 13 categories, 7-second steal window, 2–4 players with any mix of human and CPU, multiplayer via Firestore, dual dice themes — toon dice free, chrome dice Pro). |
| May 16 | Session 47: **v2.1.0 prep — dice roll audio + five audit fixes + multiplayerTrivia test file.** Wired the ElevenLabs-generated `Five_dice.mp3` dice roll SFX into `gameSounds.ts` registry and replaced `playGameSound('tap')` with `playGameSound('diceRoll')` in `DiceGameScreen.tsx` (reactive `prevDiceKeyRef` effect for both human and CPU rolls) and `MultiplayerDiceGame.tsx` (`handleRoll` + reactive effect). Discovered mid-session that the original prompt's MultiplayerDiceGame edits never landed — caught by Session 47 triple audit, all three auditors flagged, Group A re-issued the edits. Five tracked audit fixes resolved: (1) `joinGame` race condition + missing `.exists()` guard — wrapped in `runTransaction`, post-update re-read eliminated; (2) `joinTriviaGame` same pattern + `assertBelowActiveGameLimit` moved outside the transaction per Session 47 second-pass audit; (3) `cloudCheckers.isCloudResponse` validation tightened to verify `captured` array elements are length-2 numeric tuples (was only checking `Array.isArray`); (4) HomeScreen outer wrapper changed from `View` to `ScrollView` for small-device scrollability — inner Today `ScrollView` continues via `nestedScrollEnabled`; (5) `__tests__/multiplayerTrivia.test.ts` created from scratch (was missing entirely) — mirrors `multiplayer.test.ts` mock with `runTransaction` baked in, 7 `describe` blocks covering every public function in the service. Process miss: `npx jest` was left out of Group A/B/C verification blocks — caught when post-fix jest run on `multiplayer.test.ts` failed 7 `joinGame` tests because the existing Firestore mock didn't include `runTransaction`. Mock fix prompt added `runTransaction` to the existing `jest.mock('@react-native-firebase/firestore')` block. Standing rule reaffirmed: `npx tsc --noEmit` AND `npx jest` in every code-change verification block, no exceptions. Triple audit (Codex + Claude personal + Gemini) found 4 real issues + 1 out-of-scope. Real: the missed MultiplayerDiceGame sound fix (P1 unanimous), captured-array validation gap (P1 unanimous), `joinTriviaGame` transaction inconsistency (P2 — Codex + Gemini), missing `joinTriviaGame` auth/Pro/limit tests (P2). Out of scope: HomeScreen welcome overlay hardcoded colors (Gemini flagged — real finding, but pre-existing code untouched by this session's ScrollView wrap). Group A + Group B fix prompts both landed clean. Version bumped to v2.1.0 (versionCode 47). Dev merged to main as the milestone checkpoint. **AAB not yet built — deferred to a follow-up session.** Off-topic but worth noting: drafted 10 new Alarm Guy character lines this session (mix of absurd-commitment, philosophical-cut, and hammer-flat styles) plus a content-rating discussion that landed on "Teen, not Mature" — Teen unlocks the language the brand needs (damn, hell, ass, shit, anus, savage sarcasm) without dropping out of family-device filters. New voice clip recording session queued. |
| May 12 | v2.1.0 (versionCode 47) shipped to Play Store. Includes dice roll audio wired across single and multiplayer dice game, five tracked audit fixes (joinGame/joinTriviaGame transactions, cloudCheckers validation tightened, HomeScreen scrollable on small devices), new multiplayerTrivia.test.ts file, and runTransaction mock added to multiplayer.test.ts. |

...
export async function scheduleReminderNotification(
  ...
  if (isRecurring && days.length > 0 && days.length < 7) {
    ...
    for (const day of days) {
      ...
      // Early completion: if this day's trigger is today, push it forward
      // by 7 days (same day next week). Each day in the schedule has its
      // own trigger, so we only move THIS day — the others are untouched.

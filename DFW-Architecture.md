...
### New/Modified Files in Session 32

| File | Status | Purpose |
|------|--------|---------|
| `functions/` | NEW (subproject) | Firebase Cloud Functions as a separate Node 20 TypeScript project with its own `package.json` + `tsconfig.json`. `firebase-admin` + `firebase-functions` dependencies. Deploys via `firebase deploy --only functions`. Metro skips this directory via the new `metro.config.js` blockList. |
| `functions/src/checkersEngine.ts` | NEW | Pure-TS checkers engine adapted from `src/services/checkersAI.ts` — board types, move generation with forced capture + multi-jump + promotion-stops-chain, same improved evaluation (mobility, support, formation, endgame scaling, king edge penalty), minimax with alpha-beta + TT + killers. Does NOT include `getAIMove`/`DIFFICULTY_LEVELS`/`findBestMove` — the cloud only needs `getTopMoves()`. Defaults: `maxDepth=20`, `timeLimitMs=6000`, `count=5`. Server CPU makes TS deep enough without WASM. |
| `functions/src/index.ts` | NEW | `onRequest` HTTPS function `checkersAI` at `checkersai-kte3lby5vq-uc.a.run.app`. `us-central1`, 512MiB, 30s timeout, maxInstances 10, `cors: true`. Validates `{ board: Board, turn: PieceColor }` via `isValidBoard` + `isValidTurn` guards, calls `getTopMoves(board, turn, 20, 6000, 5)`, returns `{ moves: RankedMove[] }`. Public unauth endpoint for now — App Check deferred to pre-Pro launch. |
| `functions/package.json` | NEW | Node 20, `firebase-admin ^13.8.0`, `firebase-functions ^7.2.5`. `build` script is `tsc`; `predeploy` in `firebase.json` runs `npm --prefix $RESOURCE_DIR run build` before deploy. |
| `firebase.json` | NEW | Single codebase `functions` source, `disallowLegacyRuntimeConfig: true`, `predeploy: npm run build`. Lint predeploy intentionally omitted — build catches the errors that matter. |
| `metro.config.js` | MODIFIED | Blocks `/functions\/.*/` from the Metro bundle so the Cloud Function sub-project doesn't leak into the client app. `blockList` now spreads the existing Expo default via `Array.isArray(config.resolver.blockList) ? config.resolver.blockList : []` before appending — preserves any upstream exclusions. `wav` asset ext retained from Session 23. |
| `src/services/cloudCheckers.ts` | NEW | Client service. `getCloudCheckersMove(board, turn, pickRange)` does connectivity check → POST to the Cloud Function with `{ board, turn }` → 8s `AbortController` timeout → HTTP status check → `isCloudResponse` shape guard → clamp `pickRange` to available results → pick one rank uniformly in the clamped band → return the selected `CheckersMove`. Returns **null on any failure** (offline, timeout, non-OK HTTP, malformed response, empty moves) so the caller can fall through. Mirrors the `cloudStockfish.ts` pattern. Local `PickRange` type shadows the one in `checkersAI.ts` (identical shape; see P2 in audit notes). |
| `src/services/checkersAI.ts` | MODIFIED | Evaluation improved: mobility approximation (`mobilityScore` — open adjacent squares per piece, `-8` if trapped), piece support (`isSupported` — backward-diagonal friend, plus all-diagonals for kings), connected formation (`hasConnectedFriend`), endgame scaling (`totalPieces <= 8` → king value 200 instead of 160, king center bonus 18 vs 10, non-king advancement × 1.5), king edge penalty (`-5`) + double-corner penalty (`-3`). New exports: `getTopMoves(board, turn, maxDepth=14, timeLimitMs=5000, count=5): RankedMove[]` — iterative deepening scoring every root move at each completed depth, used both locally (indirectly) and mirrored on the cloud. New `PickRange` interface. `DIFFICULTY_LEVELS` extended with `cloudPickRange: PickRange`: Beginner `{2,4}`, Casual `{1,3}`, Intermediate `{0,2}`, Advanced `{0,1}`, Expert `{0,0}`. Local `getAIMove` unchanged — cloud sits in front in `useCheckers`. |
| `src/hooks/useCheckers.ts` | MODIFIED | AI move pipeline now **cloud → local fallback** (checkers has no opening book). `triggerAIMove` becomes async: session check → 400ms outer delay → inner `setTimeout(0)` yield → `await getCloudCheckersMove(b2, aiTurn, level.cloudPickRange)` → post-await re-checks `sessionIdRef.current === currentSession` AND `boardRef.current === b2` before applying. If cloud returns null, falls through to `getAIMove(b2, aiTurn, level)` (local minimax). Applies the move, plays capture/promote/move sound, bumps version, saves, checks game over. |
| `src/screens/TriviaScreen.tsx` | REWRITTEN | Full rewrite around the new parent/subcategory type system. `CATEGORY_IMAGES: Record<TriviaParentCategory, ImageSourcePropType>` for the 8 parent tiles. `handleCategoryTap` sets the parent, **resets `selectedSubcategory` to `'all'`** (P1 audit fix — prevents stale sub from a previous category corrupting the new round), then opens `SubcategoryPickerModal` for multi-sub parents (picker is skipped entirely for single-sub parents — general/geography/mythFiction). `selectQuestions` branches on `subcategory !== 'all'` (pulls from `getQuestionsForSubcategory`), `category === 'general'` (pulls from `getAllQuestions` — "grab bag"), else `getQuestionsForCategory(category)`. Seen-question composite keys: `category_subcategory` when a specific sub is selected, `category` alone when 'all'. 30s `checkConnectivity` polling drives the top-right globe badge (live cloud vs offline). Difficulty filter + speed option pills retained. |
| `src/components/SubcategoryPickerModal.tsx` | NEW | Modal rendered from `TriviaScreen` when the tapped parent has >1 subcategories. `PARENT_IMAGES` + `SUBCATEGORY_IMAGES` + `PARENT_LABELS` maps local to the modal. Computes live counts via `useMemo` on `[category]`: total for the parent (`getQuestionsForCategory`) and per-sub total + easy/medium/hard breakdown (`getQuestionsForSubcategory`). First tile is "All {parent}" (uses `PARENT_IMAGES[category]`); remaining tiles are per-sub with count + difficulty row. `onSelect(sub \| 'all')` → caller sets `selectedSubcategory` and closes modal. Backdrop + X close without selecting — the parent component's handleCategoryTap already pre-reset sub to 'all' so dismissal yields the "All parent" behavior. |
| `src/types/trivia.ts` | REWRITTEN | New type system. `TriviaParentCategory` union (8 values), `TriviaSubcategory` union (19 values), `PARENT_TO_SUBS: Record<TriviaParentCategory, TriviaSubcategory[]>`, `SUBCATEGORY_LABELS: Record<TriviaSubcategory, string>`. `TriviaQuestion` gained required `subcategory: TriviaSubcategory` field. `TriviaStats.categoryStats` keyed by `TriviaParentCategory`. Legacy `TriviaCategory` alias kept as `= TriviaParentCategory` for any lingering imports. |
| `src/data/triviaBank.ts` | NEW | Barrel aggregating 8 parent-category bank files (`triviaBank_general`, `_popCulture`, `_scienceTech`, `_historyPolitics`, `_geography`, `_sportsLeisure`, `_gamingGeek`, `_mythFiction`). Exports `getAllQuestions()`, `getQuestionsForCategory(cat)`, `getQuestionsForSubcategory(sub)`, `getQuestionCount(cat)`, `getSubcategoryCount(sub)`, `TRIVIA_CATEGORIES` (display list — moved here from the deleted legacy `triviaQuestions.ts`). Total 1,623 questions. |
| `src/data/triviaBank_general.ts` etc. (8 files) | NEW | Per-parent question banks. Each exports a `default TriviaQuestion[]`. Question IDs follow the `category_NNN` pattern (validated by test). Food + kids questions remapped to `general` — no OpenTDB-less dead categories remain. |
| `src/data/triviaQuestions.ts` | DELETED | Legacy re-export. All consumers moved to `triviaBank.ts`; `TRIVIA_CATEGORIES` migrated to the new barrel. |
| `src/services/triviaAI.ts` | MODIFIED | OpenTDB mapping now covers all 8 parent categories: `general: 9`, `popCulture: [11,12,14,26]`, `scienceTech: [17,18,30,19]`, `historyPolitics: [23,24,25]`, `geography: 22`, `sportsLeisure: [21,16,28]`, `gamingGeek: [15,29,31]`, `mythFiction: [20]`. Multi-ID categories pick a random OpenTDB id per fetch. Online questions get a default `subcategory` assignment (first entry in `PARENT_TO_SUBS[category]`) — OpenTDB doesn't expose subcategory-level queries. |
| `src/services/triviaStorage.ts` | MODIFIED | `ALL_CATEGORIES` list updated to the 8 new parent categories. Old stats for deleted categories (food, kids) silently drop on migration — `validateStats` rebuilds `categoryStats` keyed by the current `ALL_CATEGORIES` list. Seen-question keys accept either `category` or `category_subcategory` composites as opaque strings; no per-key validation — composite format is an implementation detail of the screen. |
| `src/screens/GamesScreen.tsx` | MODIFIED | 30s `checkConnectivity` polling inside `useFocusEffect` drives per-card globe badges. Online-capable cards (Daily Riddle, Chess, Checkers, Trivia) render the live `isOnline` toggle (tired wifi globe vs dead X globe); offline-only cards (Sudoku, Memory Match) render the static offline globe; Trophies has no globe. Card `accessibilityLabel` now merges globe state into the parent `TouchableOpacity` (P2 audit fix — inner `<View>` labels were ignored by the grouping). |
| `src/hooks/useEntitlement.ts` | MODIFIED | `fetchProducts` no longer single-shot. Wrapped in a retry-with-backoff loop: up to 4 total attempts (initial + 3 retries), 1s/2s/4s exponential delays, retries only on "Billing client not ready" / "not ready" error messages, all other failures fail fast. `cancelled` flag in the cleanup function prevents state updates after unmount. Fixes the cold-start race where `connected` fires true before the Play Billing client has finished its handshake. |
| `src/screens/MemoryScoreScreen.tsx` | MODIFIED | `TRIVIA_CATEGORIES` import moved from deleted `triviaQuestions.ts` to `triviaBank.ts`. |
| `__tests__/cloudCheckers.test.ts` | NEW | 8 tests covering `getCloudCheckersMove`: offline → null (fetch never called), online happy path with mock response, `pickRange {4,4}` picks 5th-best, HTTP 500 → null, malformed `moves: []` → null, network error → null, rank clamping when fewer moves returned than requested, request body shape (URL contains `checkersai`, POST method, body.turn + 8×8 board). Mocks `fetch` + `checkConnectivity`. |
| `__tests__/triviaBank.test.ts` | NEW | 14 tests covering the 1,623-question bank: ≥1500 total, unique IDs, ≥99% unique question text (tolerates legitimately similar wording), every question has a valid category, valid subcategory for its parent, valid difficulty + type, multiple = 3 incorrect answers, boolean = 1 incorrect answer, every parent has questions, every subcategory has questions, non-empty question text + correct answer, correct answer is not in incorrect answers, ID matches `category_NNN` pattern. |
| `__tests__/checkersAI.test.ts` | MODIFIED | New describe blocks for improved evaluation heuristics (supported vs isolated, king edge vs center, mobility blocked vs open, endgame finite, still ±100000 with no pieces) and for `getTopMoves` (≤5 moves for initial board, sorted best-first per side, single-legal position returns 1 move, empty for no-moves, every returned move is legal) and for `cloudPickRange` model (0 ≤ min ≤ max ≤ 4, higher difficulty = lower picks). Import adds `RankedMove`, `PickRange`, `getTopMoves`. |

### Session 32 Cloud Checkers AI — Pipeline

The Session 18 local checkers engine still ships in full (minimax + alpha-beta + TT + killers + iterative deepening + improved evaluation), but Session 32 puts a Firebase Cloud Function in front of it as the primary move source for all 5 difficulty levels. Same pattern as Session 31's cloud Stockfish: difficulty is **move quality** (which rank band to pick), not search depth.

**Priority order** (per move): **cloud Cloud Function → local minimax**. Checkers has no opening book — games diverge fast enough that a curated book wouldn't add much.

1. **Cloud Cloud Function first.** `getCloudCheckersMove(board, turn, level.cloudPickRange)` does: `checkConnectivity()` gate → `POST https://checkersai-kte3lby5vq-uc.a.run.app` with `{ board, turn }` → `AbortController` 8-second timeout → response OK check → `isCloudResponse` shape guard (expects `{ moves: [{ move: {...}, score }, ...] }` non-empty) → clamps the level's `cloudPickRange` to available results (in case the cloud returned fewer than 5) → picks one rank uniformly in the clamped band via `Math.floor(Math.random() * span)` → returns the selected `CheckersMove`. Returns **null on any failure** (offline, timeout, non-2xx, malformed, empty moves) so the client can fall through.
2. **Cloud Function internals.** `functions/src/checkersEngine.ts` is a copy of the local engine — same types, same move generation (forced capture + multi-jump + promotion-stops-chain), same improved evaluation (mobility approximation, piece support, connected formation, endgame scaling, king edge penalty, double-corner penalty), same minimax with TT + killers. Runs `getTopMoves(board, turn, 20, 6000, 5)` — deeper search than the local engine (maxDepth 14, 5s) because server CPU makes it affordable. Iterative deepening scores every root move at each completed depth; only the deepest fully-completed iteration is returned so every entry in the ranked list is from the same depth.
3. **Local fallback.** If the Cloud Function returns null (offline, slow, down, malformed), `useCheckers.triggerAIMove` falls through to `getAIMove(board, turn, level)` which runs the local iterative-deepening minimax with the level's `minDepth`/`maxDepth`/`timeLimitMs`/`randomness`. Offline users get the same experience as before Session 32 — the local engine's evaluation improvements (mobility, support, formation, endgame scaling, edge penalty) still apply.

**Async gap handling.** The cloud fetch is the only async step in the AI move pipeline. After `await getCloudCheckersMove(...)`, `useCheckers.triggerAIMove` re-checks `sessionIdRef.current === currentSession` (guards against resign/newGame/unmount) AND `boardRef.current === b2` (guards against the board being swapped mid-request). If either check fails, the callback bails early — no stale AI move applied.

**Free for everyone.** The Cloud Function has no auth/App Check yet — it's a public unauth endpoint for now. Cloud Checkers is part of the free tier (same stance as cloud Stockfish in Session 31). Pro differentiation is "unlimited rounds," not "better AI." App Check is deferred to pre-Pro launch; once added, every client request will carry an attestation token so only real DFW builds can invoke the function.

**Two copies of the engine.** The client's `src/services/checkersAI.ts` and the function's `functions/src/checkersEngine.ts` are near-identical TypeScript. They can drift. The cloud version only needs `getTopMoves` (no `getAIMove`/`DIFFICULTY_LEVELS`/`findBestMove`), so most of the file is shared. If the evaluation heuristic changes in the client, the Cloud Function needs the same edit and a redeploy — missing this would cause the cloud to return moves based on slightly different scoring, leading to subtle play-style drift. A future session could extract a shared package or symlink the engine; for now, the copy is small enough to manage manually.

### Session 32 Trivia Overhaul — Parent + Subcategory System

Sessions 1–31 treated trivia as a flat list of categories. Session 32 introduces a two-level taxonomy — 8 **parent categories** for the main tile grid, each containing 1–4 **subcategories** for a second-level picker.

- **`TriviaParentCategory`** (8 values): `general, popCulture, scienceTech, historyPolitics, geography, sportsLeisure, gamingGeek, mythFiction`. Rendered as the tile grid on `TriviaScreen`.
- **`TriviaSubcategory`** (19 values): e.g. `generalKnowledge, film, music, television, celebrities, scienceNature, computers, gadgets, mathematics, history, politics, art, geography, sports, boardGames, vehicles, videoGames, comicsAnime, mythologyBooks`.
- **`PARENT_TO_SUBS`** maps each parent to its subcategory list. Parents with a single sub (`general`, `geography`, `mythFiction`) **skip the modal entirely** — `handleCategoryTap` treats them as instant "All {parent}" selections. Parents with >1 subs open `SubcategoryPickerModal` for the user to pick a sub or "All {parent}".
- **`SUBCATEGORY_LABELS`** maps each sub to its display string ("Film & Movies", "Science & Nature", "Computers & Tech", "Comics, Anime & Cartoons", etc.) — avoids putting display text in the taxonomy itself.

**1,623 questions in 8 data files.** Each parent lives in its own `src/data/triviaBank_{parent}.ts` file exporting a `TriviaQuestion[]`. `src/data/triviaBank.ts` is the barrel: imports each file, unions them into `ALL_QUESTIONS`, and exposes `getAllQuestions`, `getQuestionsForCategory`, `getQuestionsForSubcategory`, `getQuestionCount`, `getSubcategoryCount`. Every question has `id: category_NNN`, `category: TriviaParentCategory`, `subcategory: TriviaSubcategory`, `difficulty`, `type`, `question`, `correctAnswer`, `incorrectAnswers[]`.

**SubcategoryPickerModal** renders per-sub counts live via `useMemo` on `[category]`. First tile is "All {parent}" using `PARENT_IMAGES[category]`; rest are per-sub tiles with `SUBCATEGORY_IMAGES[sub]`, total count, and a three-column difficulty breakdown (`{n} easy`, `{n} med`, `{n} hard`). 11 new trivia-specific DFW icons were added to `assets/trivia/` for the new subcategories + updated parent tiles.

**Selection flow.** `handleCategoryTap(catId)` — (1) haptic + tap sound, (2) `setSelectedCategory(catId)`, (3) **`setSelectedSubcategory('all')`** (P1 fix — before Session 32's audit, a stale sub from a previous category could persist when the user dismissed the picker without selecting; now every category tap resets to 'all' so dismissing the picker yields the "All {new parent}" behavior), (4) if the parent has a single sub, skip the modal; otherwise open `SubcategoryPickerModal`. Selection inside the modal overrides 'all' with the picked sub. "Start Round" at the bottom of the screen fires `startRound(selectedCategory)` which reads `selectedSubcategory` to build the question pool.

**`selectQuestions` branch table** (for offline rounds):
1. `subcategory !== 'all'` → `pool = getQuestionsForSubcategory(sub)` — strict sub filter.
2. `category === 'general'` → `pool = getAllQuestions()` — "grab bag" across the whole 1,623-question bank.
3. otherwise → `pool = getQuestionsForCategory(category)` — all subs under this parent.

**Seen-question composite keys.** `addSeenQuestionIds(key, ids)` and `getSeenQuestionIds(key)` accept opaque string keys. `TriviaScreen` composes `'{category}_{subcategory}'` when a specific sub is selected, and `'{category}'` alone when 'all' is selected. This preserves granular per-view recycling (each "view" cycles its own seen set) at the cost of some cross-view dedup — a question seen via "All Pop Culture" isn't recognized as seen when the user next plays "Film" only. Logged as a known P2 in the audit notes; intentional for now.

**Online trivia unchanged in shape but remapped.** `triviaAI.ts`'s `CATEGORY_MAP` now covers all 8 parent categories (food + kids are gone — their questions were remapped into `general`), so every parent has an OpenTDB online backing. Online questions get a default `subcategory` (the first entry in `PARENT_TO_SUBS[parent]`) because OpenTDB doesn't expose subcategory-level queries.

### Session 32 Online/Offline Globe Indicators

Users frequently asked "is this game using the cloud right now?" — Session 32 adds a persistent visual answer on `GamesScreen` and `TriviaScreen`.

- **`GamesScreen`** polls `checkConnectivity()` on focus and every 30s while the screen is focused. Each game card renders a globe badge in the top-right corner:
  - **Online-capable games** (Daily Riddle, Chess, Checkers, Trivia) toggle between a tired wifi globe (`icon-globe.webp`, connectivity true) and a dead X globe (`offline_globe.webp`, connectivity false).
  - **Offline-only games** (Sudoku, Memory Match) always show the static offline globe.
  - **Trophies** has no globe.
- **`TriviaScreen`** uses the same 30s polling pattern with an absolute-positioned top-right globe badge during the category phase, mirroring the tile-level badges on GamesScreen.
- **Accessibility.** The globe `<View>` no longer carries its own `accessibilityLabel` — React Native's TouchableOpacity grouping ignores inner child labels. The state ("cloud features active" / "offline mode") is now merged into the parent `TouchableOpacity`'s `accessibilityLabel` so screen readers announce it with the card's primary label.
- **Custom assets.** Both globes are DFW-style original art (tired wifi globe + dead X globe), matching the anthropomorphic game-icon voice rather than stock status icons.

### Session 32 IAP Retry — `fetchProducts` Cold-Start Fix

Users on cold app-start sometimes saw `[Expo-IAP] Error fetching products: Billing client not ready`. The `useEntitlement` effect fired `fetchProducts` as soon as `connected` flipped true, but Play Billing sometimes hadn't fully handshaked yet at that moment.

The `fetchProducts` effect now runs a retry-with-backoff loop: initial attempt + up to 3 retries, 1-second / 2-second / 4-second exponential delays, retries only on error messages containing "Billing client not ready" or "not ready". All other errors fail fast (no retry storm on structural failures). A `cancelled` flag in the cleanup function prevents state updates after unmount — the in-flight sleep still runs to completion, but subsequent loop iterations short-circuit.
